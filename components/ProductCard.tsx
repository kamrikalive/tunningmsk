...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  // Ссылка на видео-элемент для управления воспроизведением
  const videoRef = useRef<HTMLVideoElement>(null);

  // 1. Собираем весь медиа-контент (Картинки + Видео в конце)
  ...
  // Сброс на первый слайд при закрытии модалки или уходе мыши (опционально)
  // useEffect(() => { setCurrentSlideIndex(0); }, [isDetailOpen]);
  ...
  // Быстрый переход к видео при клике на иконку
  const jumpToVideo = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (product.video) {
      changeSlide(slides.length - 1); // Видео всегда последнее
      // Автоплей сработает через атрибут autoPlay, так как компонент перерендерится
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          {/* Текущий слайд (Картинка или Видео) */}
          <div className="relative w-full h-full">
             {isVideoSlide(currentSlideIndex) ? (
               ...
               <video
                 ...
                 controls // Разрешаем управлять (плей/пауза)
                 autoPlay // Начинаем играть сразу, как переключились
                 muted // Важно: без muted автоплей может не сработать в некоторых браузерах (но можно убрать, если хотите звук сразу)
                 playsInline
                 onClick={(e) => e.stopPropagation()} // Чтобы клик по видео не открывал модалку
               />
             ) : (
               // --- КАРТИНКА ---
               <Image
          ...
          {/* СТРЕЛКИ (Показываем только если слайдов > 1) */}
          {slides.length > 1 && (
            <>
              <button 
                onClick={prevSlide}
                className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-black/50 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-blue-600 z-10"
              >
              ...
              <button 
                onClick={nextSlide}
                className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-black/50 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-blue-600 z-10"
              >
                  ...
                  <button
                    ...
                    className={`rounded-full transition-all shadow-sm ${
                      idx === currentSlideIndex 
                        ? 'bg-blue-500 w-4 h-2' 
                        : isVideoSlide(idx) 
                          ? 'bg-red-500/70 hover:bg-red-500 w-2 h-2' // Красная точка для видео
